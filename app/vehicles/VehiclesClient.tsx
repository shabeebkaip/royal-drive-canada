...
interface VehiclesPageContentProps {
  initialBrands: BrandAPI[];
  initialBodyTypes: VehicleTypeAPI[];
  initialFuelTypes: FuelType[];
  initialTransmissions: TransmissionType[];
}
...
const VehiclesPageContent = ({ 
  ...
  // Data states - Initialize with server-side data
  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadingMore, setLoadingMore] = useState(false);
  const [brands] = useState<BrandAPI[]>(initialBrands);
  const [models, setModels] = useState<ModelAPI[]>([]);
  const [fuelTypes] = useState<FuelType[]>(initialFuelTypes);
  const [transmissions] = useState<TransmissionType[]>(initialTransmissions);
  const [bodyTypes] = useState<VehicleType[]>(
    initialBodyTypes.map((vt) => ({
      id: vt._id,
      name: vt.name,
